...
export default function Rekap12BulanPage() {
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
      <div>
        <h2 className="text-2xl font-bold text-slate-900">Rekap 12 Bulan</h2>
        <p className="text-slate-600">Rekap kegiatan tahun {tahun}</p>
      </div>
          ...
          {isLoading ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-2 py-2 text-left font-semibold text-slate-700 sticky left-0 bg-slate-50">Jenis Kegiatan</th>
                  {bulanRange.map(bulan => (
                    <th key={bulan} className="px-2 py-2 text-center font-semibold text-slate-700">{bulan}</th>
                  ))}
              ...
              <tbody className="divide-y divide-slate-200">
                {activities.map(activity => (
                  <tr key={activity.id}>
                    <td className="px-2 py-2 text-slate-900 text-xs sticky left-0 bg-white">{activity.nama_kegitan}</td>
                    {bulanRange.map(bulan => (
                      <td key={bulan} className="px-2 py-2 text-center text-slate-600">{getValue(activity.id, bulan)}</td>
                    ))}
                    <td className="px-2 py-2 text-center font-medium text-slate-900">{getTotal(activity.id)}</td>
                  </tr>
                ))}
                <tr className="bg-slate-50 font-semibold">
                  <td className="px-2 py-2 text-slate-900 sticky left-0 bg-slate-50">TOTAL</td>
                  {bulanRange.map(bulan => (
                    <td key={bulan} className="px-2 py-2 text-center text-slate-900">{activities.reduce((sum, a) => sum + getValue(a.id, bulan), 0)}</td>
                  ))}
                  <td className="px-2 py-2 text-center text-slate-900">{activities.reduce((sum, a) => sum + getTotal(a.id), 0)}</td>
                </tr>
              </tbody>
            </table>
          )}
